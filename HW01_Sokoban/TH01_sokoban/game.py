...
class Game:
    ...
    def auto_move(self):
        ...
        file_path = "assets/sokobanSolver/{}/Solverlevel_{}.txt".format(strategy[1], self.index_level)
        with open(file_path, 'w+') as solver_file:
            solver_file.write("Runtime: %f\n" %strategy[2])
            solver_file.write("N.o steps: %d\n"%len(strategy[0]))
            solver_file.write('Result: %s' %strategy[0])
        if strategy[0] is not None:
            try:
                _thread.start_new_thread( move, ("Thread-1", 2, strategy[0]) )
            except:
