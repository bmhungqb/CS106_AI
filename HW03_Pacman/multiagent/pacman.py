...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    res = [['Loss', 'Win'][int(w)] for w in wins]
    score = (sum(scores) / float(len(scores)))
    file_path = "C:/Users/bmhun/Documents/TaiLieuHocTapDaiHoc/Year2/HK_II/UIT/AI/Homework/HW03_Pacman/result.txt"
    with open(file_path, 'a') as file:
        file.write('(score_res_runtime)ExpectimaxAgent_scoreEva_powerClassic: %s %s ' %(score,res[0]))
    file.close()
    return games
